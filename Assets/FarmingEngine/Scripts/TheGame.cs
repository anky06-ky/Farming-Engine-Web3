...
namespace FarmingEngine
{
    ...
    public class TheGame : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            // Optionally, you can hook into afterLoad/afterNewGame here if needed for Web3.
        }
        ...
        private void Start()
        {
            ...
            // --- Web3: sync NFT-owned items into inventories after world and player are ready ---
            // Note: Auto-sync disabled. User must manually click "Sync NFT Inventory" button in Web3DebugPanel (F9)
            // SyncNFTInventory();
        }

        public void SyncNFTInventory()
        {
            PlayerData pdata = PlayerData.Get();
            if (pdata == null)
                return;
            ...
            {
                Debug.Log("[Web3] SyncNFTInventory skipped: wallet_address is empty.");
                return; // No wallet connected for this save, skip Web3 sync
            }

            Debug.Log("[Web3] SyncNFTInventory started for wallet: " + wallet);

            Web3BackendClient.Get().FetchOwnedNFTs((nftList) =>
            {
                if (nftList == null || nftList.items == null)
                    return;

                // For simplicity, sync into main inventory of player 0
                InventoryData inventory = InventoryData.Get(InventoryType.Inventory, 0);
                if (inventory == null)
                    return;

                // Remove existing NFT items first (basic approach: any item that is NFTItemData)
                List<int> slotsToClear = new List<int>();
                for (int i = 0; i < inventory.size; i++)
                {
                    InventoryItemData invItem = inventory.GetInventoryItem(i);
                    if (invItem != null)
                    {
                        ItemData idata = ItemData.Get(invItem.item_id);
                        if (idata is NFTItemData)
                            slotsToClear.Add(i);
                    }
                }
                foreach (int slot in slotsToClear)
                    inventory.RemoveItemAt(slot, int.MaxValue);

                // Add all owned NFTs
                Debug.Log("[Web3] Processing " + nftList.items.Count + " NFT items for inventory sync");
                foreach (var nft in nftList.items)
                {
                    if (string.IsNullOrEmpty(nft.itemId))
                    {
                        Debug.LogWarning("[Web3] Skipping NFT with empty itemId");
                        continue;
                    }

                    Debug.Log("[Web3] Looking for item: " + nft.itemId);
                    NFTItemData nftItem = ItemData.Get(nft.itemId) as NFTItemData;
                    
                    // Fallback: if not NFTItemData, try regular ItemData (for demo purposes)
                    ItemData regularItem = null;
                    if (nftItem == null)
                    {
                        regularItem = ItemData.Get(nft.itemId);
                        if (regularItem != null)
                        {
                            Debug.Log("[Web3] Found regular ItemData (not NFTItemData): " + nft.itemId + ". Adding to inventory anyway for demo.");
                        }
                        else
                        {
                            Debug.LogWarning("[Web3] Item not found in game data: " + nft.itemId + ". Create NFTItemData or ItemData ScriptableObject with this id.");
                            continue;
                        }
                    }
                    
                    ItemData itemToAdd = nftItem != null ? nftItem : regularItem;

                    Debug.Log("[Web3] Adding item to inventory: " + itemToAdd.id + " (slot will be assigned)");
                    // Use durability from item data; quantity is 1 per NFT
                    int slot = inventory.AddItem(itemToAdd.id, 1, itemToAdd.durability, UniqueID.GenerateUniqueID());
                    Debug.Log("[Web3] Item added to inventory at slot: " + slot);

                    // Optionally you could track objectId per-slot via PlayerData custom strings or another map
                    // For now we just ensure presence in inventory reflects on-chain ownership.
                }
                Debug.Log("[Web3] NFT inventory sync completed");
                
                // Mark NFT inventory as synced to enable game actions
                Web3WalletValidator.MarkNFTInventorySynced();
            },
            (error) =>
            {
                Debug.LogWarning("Web3 NFT sync failed: " + error);
            });
        }
