...
namespace FarmingEngine
{
    ...
    public class PlayerCharacterCraft : MonoBehaviour
    {
        ...
        {
            // --- Web3: Block ALL crafting if wallet not valid (not set or not synced) ---
            if (!Web3WalletValidator.IsWalletValid())
            {
                Debug.LogWarning("[Web3] Cannot craft: Wallet not set or NFT inventory not synced.");
                return null; // Block crafting
            }

            // --- Web3: Additional check for NFT items ---
            NFTItemData nftItem = item as NFTItemData;
            if (nftItem != null && nftItem.autoMintOnCraft)
            {
                // Wallet validation already done above, but double-check for NFT items
                string wallet = PlayerData.Get()?.GetWalletAddress() ?? "";
                if (string.IsNullOrEmpty(wallet))
                {
                    Debug.LogWarning("[Web3] Cannot craft NFT item without wallet address: " + nftItem.id);
                    ReadPanel panel = ReadPanel.Get(0);
                    if (panel != null)
                    {
                        panel.ShowPanel("Wallet Required", "You need to connect a wallet to craft " + nftItem.title + ".\n\nPlease set your wallet address in the Web3 Debug Panel (Press F9).");
                    }
                    return null; // Block crafting
                }
            }
            ...
            if (CanCraft(item, skip_cost))
            {
                ...
                // --- Web3: optionally mint NFT for special items (e.g. legendary tools) ---
                if (nftItem != null && nftItem.autoMintOnCraft)
                {
                    Debug.Log("[Web3] Auto-minting NFT for crafted item: " + nftItem.id);
                    Web3BackendClient.Get().MintNFT(nftItem.id, (objectId) =>
                    {
                        Debug.Log("[Web3] NFT minted successfully! ObjectId: " + objectId);
                        // Here you can link the minted objectId to game state (e.g. last crafted NFT)
                        PlayerData.Get().SetCustomString("last_minted_nft_objectId", objectId);
                        
                        // Show notification to player
                        ReadPanel panel = ReadPanel.Get(0);
                        if (panel != null)
                        {
                            panel.ShowPanel("NFT Minted!", "Your " + nftItem.title + " has been minted as an NFT!\nObject ID: " + objectId);
                        }
                    },
                    (error) =>
                    {
                        Debug.LogError("[Web3] MintNFT failed for item " + nftItem.id + ": " + error);
                        // Show error notification
                        ReadPanel panel = ReadPanel.Get(0);
                        if (panel != null)
                        {
                            panel.ShowPanel("Mint Failed", "Failed to mint NFT: " + error);
                        }
                    });
                }
